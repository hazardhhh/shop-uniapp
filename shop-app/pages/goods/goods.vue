	...
	<view>
		...
		<view class="swiper-box">
			<swiper circular="true" autoplay="true" @change="swiperChange">
				<swiper-item v-for="swiper in goodsData.productPicList" :key="swiper.id">
					<image :src="swiper.pic"></image>
				</swiper-item>
			</swiper>
			<view class="indicator">{{currentSwiper+1}}/{{goodsData.productPicList.length}}</view>
		</view>
		<!-- 标题 价格 -->
...
<script>
	...
	export default {
		...
		onLoad(option) {
			this.showBack = false;
			console.log("xxx:", option.id); //打印出上个页面传递的参数。
			api.getProductById(option.id).then(data => {
				// 因为后台查询的数据没有这些属性，给动态加进去默认值
				data.data.service = this.goodsData.service;
				data.data.spec = this.goodsData.spec;
				data.data.comment = this.goodsData.comment;
				data.data.number = 1;
		...
		methods: {
			...
			hideSpec() {
				...
				api.addUserCar(id,num).then(data =>{
