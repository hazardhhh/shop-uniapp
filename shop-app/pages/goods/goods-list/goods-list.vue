<template>
	<view>
		<view class="header" :style="{position:headerPosition,top:headerTop}">
			<view class="target" v-for="(target,index) in orderbyList" @tap="select(index,target.type)" :key="index" :class="[target.selected?'on':'']">
				{{target.text}}
		...
		<view class="goods-list">
			<view class="product-list">
				<view class="product" v-for="(goods) in goodsList" :key="goods.goods_id" @tap="toGoods(goods)">
					<image mode="widthFix" :src="goods.productPicList[0].pic"></image>
	...
	export default {
		data() {
			return {
				...
				orderbyList:[
					{type:1,text:"销量",selected:true,orderbyicon:false,orderby:0},
					{type:2,text:"价格",selected:false,orderbyicon:['sheng','jiang'],orderby:0},
					{type:4,text:"好评",selected:false,orderbyicon:false,orderby:0}
				],
				orderby:"sheng"
			};
		},
		onLoad: function (option) { //option为object类型，会序列化上个页面传递的参数
			...
			// 发送请求查询商品类型id对应的商品集合,默认用销量来排序
			api.homeProductByCategoryId(0,1).then(data =>{
		...
		onReachBottom(){
			...
			for(let i=1;i<=10;i++){
				...
				let p = { goods_id: goods_id, img: '/static/img/goods/p'+(goods_id%10==0?10:goods_id%10)+'.jpg', name: '商品名称-'+i, price: '￥168', slogan:'1235人付款' };
		...
		methods:{
			...
			select(index,orderType){
				...
				api.homeProductByCategoryId(this.id,orderType).then(data=>{
					this.goodsList = data.data;
				})
