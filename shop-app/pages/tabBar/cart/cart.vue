	...
	<view>
		...
		<view class="goods-list">
            ...
            <view class="row" v-for="(row,index) in goodsList" :key="index" >
				<!-- 删除按钮 -->
				<view class="menu" @tap.stop="deleteGoods(row.product.id)">
				...
				<view class="carrier" :class="[theIndex==index?'open':oldIndex==index?'close':'']" @touchstart="touchStart(index,$event)" @touchmove="touchMove(index,$event)" @touchend="touchEnd(index,$event)">
					...
					<view class="goods-info" @tap="toGoods(row)">
						<view class="img">
							<image :src="row.product.productPicList[0].pic"></image>
						</view>
						<view class="info">
...
import api from "@/request/api.js"
	...
	export default {
		...
		methods: {
			...
			getUserCar(){
				api.getUserCar().then(data =>{
					this.goodsList = data.data.carItem;
				});
			},
			...
			deleteGoods(id){
				...
				api.deleteUserCar(id);
				this.getUserCar(); // 刷新购物车
				this.sum();
				this.oldIndex = null;
				this.theIndex = null;
			},
			...
			sub(index){
				...
				api.updateUserCar(this.goodsList[index].product.id,2);
			...
			add(index){
				this.goodsList[index].num++;
				api.updateUserCar(this.goodsList[index].product.id,1);
				this.sum();
			},
			// 合计
			sum(e,index){
				...
				for(let i=0;i<len;i++){
					if(this.goodsList[i].selected) {
						if(e && i==index){
							...
							this.sumPrice = this.sumPrice + (this.goodsList[i].num*this.goodsList[i].product.price);
						}
					}
				}
