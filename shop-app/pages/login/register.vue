		...
		methods: {
			...
			getCode() {
				...
				setTimeout(() => {
					...
					// 调用后台发送验证码
					api.registerSendCode(this.phoneNumber).then(data => {
						if (data.code == 0) { // 后台验证码已发送
							uni.showToast({
								title: '验证码已发送',
								icon: "none"
							});
			...
			setTimer() {
				...
				this.Timer = setInterval(() => {
					if (holdTime <= 0) {
						...
						this.getCodeBtnColor = "#ffffff"; // 可以点击了
			...
			doReg() {
				...
				api.userRegister(param).then(data => {
					if (data.code == 0) {
						uni.showToast({
							title: '注册成功',
							icon: "none"
