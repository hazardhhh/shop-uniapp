				...
				<view class="goods-info">
					...
					<view class="info">
						...
						<view class="price-number">
							<view class="price">￥{{row.subPrice}}</view>
							<view class="number">
	...
	export default {
		...
		onShow() {
			...
			uni.getStorage({
				...
				success: (ret) => {
					...
					for (let i = 0; i < len; i++) {
						this.goodsPrice = this.goodsPrice + (this.buylist[i].num * this.buylist[i].product.price);
					}
			...
			uni.getStorage({
				...
				success:function(data){
					console.log("用户选择了",data.data.name);
					if(data.data.name){
						// 给上面的默认地址赋值
						// 赋值完成后马上清空
					}
			...
			api.getUserDefaultAddress().then(data=>{
		...
		methods: {
			...
			toPay() {
				...
				let len = this.buylist.length;
				for (let i = 0; i < len; i++) {
					goodsid.push(this.buylist[i].product.id);
				}
				if (goodsid.length == 0) {
					uni.showToast({
						title: '订单信息有误，请重新购买',
				...
				param.addressId = this.recinfo.id;
				param.prodctIdList=goodsid;
				param.remark = this.note;
				
				console.log("param:",param);
				
				api.createOrder(param).then(data=>{
